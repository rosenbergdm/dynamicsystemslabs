...
Included below are a couple of quick examples highlighting the use of \R as a calculator.  A more complete listing of basic calculation functions is provided in table~\ref{tab:arith_table}.
...
As discussed in \ref{sec:first_r_session}, variables play a key part in computational programming.  In \R, variables are defined using the \ri{variable<-value} syntax.  Here, we will discuss some of the nuances and common pitfalls encountered when using variables in \R.
...
\begin{itemize}
  ...
  \item There are a small number of reserved variable names (listed in table \ref{??}) which cannot be used.  In general, however, the names of \R functions are \emph{not} protected.  Thus, it is possible to define a variable named \Com{print} which will prevent you from using the \Com{print()} function.  Beware of this.
\end{itemize}
...
\begin{table}[htbp]\tt
  \begin{center}
    ...
    \begin{tabular}{cccc}
      ...
      \hline
    \end{tabular}
    \caption{Reserved names in \R.}
  \end{center}
\end{table}

In addition to these restrictions, style\footnote{\TODO{Talk about style / reference to style}} provides an additional list of \emph{suggestions}.
...
\begin{enumerate}
  \item \R defaults to \emph{column-major} format for matrices\footnote{\TODO{row-major vs. column-major}}
...
As discussed in the overview, keeping \R source code in text files makes maintaining and using your code much easier\footnote{A famous computer scientist once remarked that being a good programmer \emph{required} one to be a bit lazy \TODO{REFERENCE NEEDED HERE}.  While I doubt many professors would actively encourage laziness, there is no doubt that, when it comes to computer programming, the old adage of ``work smarter, not harder'' holds true.  By building a collection of source files, you accumulate a body of code which you will (hopefully) be able to use at a later date.}.  This section is intended to help you in the organization and construction of source code files.

\subsection{Overview} % (fold)
\label{sub:overview_s}

Although there are few hard and fast rules defining what and how you may use source files (any commands that may be entered into the \R interpreter may be put into a source file), the notion of \emph{style} provides \textbf{strong} suggestions as to what should and should not be done.  What follows below are general guidelines you \emph{may} wish to consider\footnote{In the end, having a consistent style helps \emph{you} more than anyone else.  You will likely spend many more hours looking at your code than anyone else, so it pays to make your code easy to read.}.  More specific and detailed guidelines are available at \hreft{http://google-styleguide.googlecode.com/svn/trunk/google-r-style.html} (Google's \R style guide) and \hreft{http://www1.maths.lth.se/help/R/RCC/} (\R Coding Conventions draft).

\R source files are generally described as being \emph{function} files or \emph{script} files.  We will explore each separately.
...
getPrimeFactors <- function(input_number) {
  ...
  max_test_to <- floor(sqrt(input_number));
...
\R function files are files that contain one (or more, if related) function definitions (see \ref{sub:example_4_functions}). and
...
\begin{enumerate}
  \item \begin{enumerate}
    \item  Starting with the fibonacci functions from the tutorial, write a new function which takes as input a number \Com{k} and returns the largest fibonacci number less than or equal to \Com{k}
    \item Write a function which takes as input three numbers (say, \Com{k}, \Com{x}, and \Com{y}) and returns either the second number (\Com{x}) or the third number (\Com{y}) depending on which is closer to \Com{k}.  (\emph{Hint:} the functions \Com{min()}, \Com{max()}, and \Com{abs()} may be helpful.  See the online documentation for their usage.)
    \item Write a function which takes as input a number \Com{k} and returns the fibonacci number with is closest to \Com{k}.
  \end{enumerate}
  \item Write a function that takes as input a vector of integers and returns a vector of those integers, sorted from smallest to largest.\footnote{\TODO{ provide a hint}}
